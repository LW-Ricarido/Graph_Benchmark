...
class CoraFeatureDataset(CoraDataset):
    ...
    def _load(self):
        ...
        if os.listdir(self.path) == []:
            ...
            adj_matrix = torch.zeros(og.num_nodes(), og.num_nodes()).to(self.device)
        ...
        if self.self_loop:
            self.g = dgl.to_simple(dgl.add_self_loop(self.g))
        self.g = self.g.to(self.device)
        logging.log(logging.INFO,"=================graph edge num:{}".format(self.g.num_edges()))
